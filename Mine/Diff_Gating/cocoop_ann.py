...
def train_one_epoch(epoch):
    ...
    for images, labels in pbar:
        ...
        n_batches += 1
        pbar.set_postfix(loss=f"{total_loss/n_batches:.4f}", gate=f"{(gate_sum/gate_steps) if gate_steps else 0:.4f}")

    pbar.close()

    # expose mean gate value for epoch in return tuple
    epoch_gate_mean = gate_sum / max(gate_steps, 1)
    return total_loss / max(n_batches, 1), epoch_gate_mean
...
def train_and_evaluate():
    ...
    for epoch in range(EPOCHS):
        ...
        # Train
        avg_loss, epoch_gate_mean = train_one_epoch(epoch)
